...
c_length = [70, 70, 69, 69, 69, 69, 68, 68, 68, 67, 67]
Y = []
for c in c_length:
    Y.append(70 - c)
...
def cal_alpha_base(x, y):
    if c - y == 0:
        return 90  # or handle it appropriately
...
for i in range(len(X)):
    ...
    # beta = (cal_beta(X[i], Y[i])) 
    beta = 180 - (cal_beta(X[i], Y[i])) # use for opposite motor
    # alpha = (90 - (alphaMain - alphaBase)) 
    alpha = 180 - (90 - (alphaMain - alphaBase)) # use for opposite motor
...
# print()
# print(betaBottomArr)
print(f"int halfOutsidePalAlphaGaitTIM[] = {{{', '.join(map(str, palAlphaGaitTIM))}}};")
print(f"int halfOutsidePalBetaGaitTIM[] = {{{', '.join(map(str, palBetaGaitTIM))}}};")
print()
