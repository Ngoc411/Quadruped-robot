...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
		int left = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_11);
		int forward = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_10);
		int back = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_1);
		int right = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_0);
		
		if (left || forward || back || right) {
			if (left) {
				PAKET[0] = 1;
			} if (forward) {
				PAKET[1] = 1;
